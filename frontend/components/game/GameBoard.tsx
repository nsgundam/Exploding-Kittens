...
function WinnerPopup({ isMe, displayName }: { isMe: boolean; displayName: string }) {
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col items-center gap-4 p-8 rounded-3xl text-center"
        style={{ width: "420px", background: "rgba(0,10,0,0.95)", border: "2px solid rgba(250,204,21,0.6)", boxShadow: "0 0 80px rgba(250,204,21,0.3), 0 24px 60px rgba(0,0,0,0.8)" }}>
        <div className="text-7xl animate-bounce">🏆</div>
        <h2 className="text-4xl font-black text-yellow-400 font-bungee uppercase tracking-wider"
          style={{ textShadow: "0 0 30px rgba(250,204,21,0.8)" }}>
          {isMe ? "คุณชนะ!" : `${displayName} ชนะ!`}
        </h2>
        <p className="text-gray-300 text-base">
          {isMe ? "เยี่ยมมาก! คุณเป็นผู้รอดชีวิตคนสุดท้าย 🎉" : `${displayName} เป็นผู้รอดชีวิตคนสุดท้าย`}
        </p>
        <button
          ...
          style={{ background: "linear-gradient(135deg, #fbbf24, #d97706)", border: "2px solid rgba(250,204,21,0.8)", boxShadow: "0 4px 0 #92400e" }}>
          กลับสู่ห้อง
        </button>
      </div>
    </div>
...
export function GameBoard({
  ...
  return (
    <main className="flex-1 flex items-center justify-center px-6 py-4 relative">
      <div className="absolute left-2 top-1/4 text-5xl opacity-5 animate-float" style={{ animationDelay: "1s" }}>🐱</div>
      <div className="absolute right-2 bottom-1/4 text-5xl opacity-5 animate-float" style={{ animationDelay: "2s" }}>💥</div>
      ...
      {eliminatedPlayerId && !winner && (() => {
        ...
        return (
          <div className="fixed inset-0 z-[2000] flex items-center justify-center">
            ...
            <div className="relative z-10 flex flex-col items-center gap-4 p-8 rounded-3xl text-center"
              style={{ width: "380px", background: "rgba(10,0,0,0.93)", border: "2px solid rgba(239,68,68,0.5)", boxShadow: "0 0 60px rgba(239,68,68,0.3), 0 24px 60px rgba(0,0,0,0.8)" }}>
              <div className="text-6xl animate-bounce">💥</div>
              <h2 className="text-3xl font-black text-white font-bungee uppercase tracking-wider"
                style={{ textShadow: "0 0 20px rgba(239,68,68,0.8)" }}>
                {isMe ? "คุณแพ้แล้ว!" : `${displayName} แพ้แล้ว!`}
              </h2>
              <p className="text-gray-400 text-base">
                {isMe ? "คุณถูกระเบิด Exploding Kitten!" : `${displayName} ถูกระเบิดหลุดออกจากเกม`}
              </p>
              <button
                ...
                style={{ background: "linear-gradient(135deg, #dc2626, #7f1d1d)", border: "2px solid rgba(239,68,68,0.5)", boxShadow: "0 4px 0 #450a0a" }}>
                ตกลง
              </button>
            </div>
          </div>
      ...
      {/* ── WINNER POPUP ── */}
      {winner && (() => {
        const isMe = roomData.players?.find(
      ...
      {roomData.status === "PLAYING" && gameLogs.length > 0 && (
        <div className="absolute left-3 top-1/2 -translate-y-1/2 w-60 z-20">
          ...
          >
            <div className="text-[10px] uppercase tracking-widest mb-1 pb-1 font-bold" style={{ color: "rgba(120,60,0,0.85)", borderBottom: "1px solid rgba(120,60,0,0.2)" }}>
              📋 Game Log
            </div>
            {gameLogs.slice(-7).map((log, i, arr) => (
              <div key={i} className="text-xs leading-snug" style={{ color: i === arr.length - 1 ? "rgba(60,30,0,0.95)" : `rgba(80,40,0,${0.4 + i * 0.12})` }}>
                {log}
              </div>
            ))}
          </div>
      ...
      <div className="w-full relative h-[520px]">
        {[5, 1, 2, 4, 3].map((seatNum) => {
          let positionClasses = "";
          if (seatNum === 5) positionClasses = "left-1/2 -top-[8%] -translate-x-1/2";
          ...
          const isHost = !!roomData.host_token && player?.player_token === roomData.host_token;

          return (
            <div key={seatNum} className={`absolute flex justify-center ${positionClasses}`}>
              <PlayerAvatar
                seat={seatNum}
                player={player}
        ...
        {roomData.status === "PLAYING" && (
          <div className="absolute left-1/2 top-[53.5%] -translate-x-1/2 -translate-y-1/2 flex items-center justify-center gap-10">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-1 px-3 py-1 rounded-full text-sm font-bold" style={{ background: "rgba(100,50,0,0.35)", border: "1px solid rgba(150,90,0,0.6)", color: "#fff8f0", fontFamily: "'Fredoka One',cursive" }}>
              ...
              <div
                ...
                onClick={() => { if (currentTurnSeat !== null && isMySeat(currentTurnSeat)) drawCard(); }}
                style={{
                  background: "linear-gradient(135deg, #8b4a1a, #5c2d0a)",
                  border: currentTurnSeat !== null && isMySeat(currentTurnSeat) ? "3px solid #f5a623" : "3px solid #c47a3a",
                  boxShadow: currentTurnSeat !== null && isMySeat(currentTurnSeat) ? "0 0 20px rgba(245,166,35,0.6)" : "none",
                }}
              >
                <div className="absolute inset-1 rounded-lg opacity-30" style={{ backgroundImage: "repeating-linear-gradient(45deg,#fff 0,#fff 1px,transparent 0,transparent 50%)", backgroundSize: "6px 6px" }} />
                <div className="absolute inset-0 flex items-center justify-center text-5xl">💣</div>
                <div className="absolute bottom-2 inset-x-0 flex justify-center">
                  <span className="text-[9px] tracking-wider" style={{ color: "rgba(245,166,35,0.8)" }}>TAP TO DRAW</span>
                </div>
              </div>
              <span className="text-xs tracking-widest uppercase font-bold" style={{ color: "#fff8f0", textShadow: "0 1px 4px rgba(0,0,0,0.5)" }}>DECK</span>
            </div>
              ...
              >
                {lastPlayedCard ? (
                  <div className="flex flex-col items-center justify-center gap-1.5 p-2">
                    ...
                    <span className="text-xs font-black tracking-wider text-center leading-none" style={{ color: CARD_CONFIG[lastPlayedCard.cardCode]?.color ?? "#f5a623" }}>
                      {CARD_CONFIG[lastPlayedCard.cardCode]?.label ?? lastPlayedCard.cardCode}
                    </span>
                    <span className="text-[9px] text-center" style={{ color: "rgba(255,240,200,0.5)" }}>{lastPlayedCard.playedByDisplayName}</span>
                  </div>
                ) : (
                  <span className="text-sm text-center leading-tight px-2" style={{ color: "rgba(255,240,200,0.4)" }}>PLAY<br />CARD</span>
                )}
              </div>
              <span className="text-xs tracking-widest uppercase font-bold" style={{ color: "rgba(255,240,200,0.6)", textShadow: "0 1px 4px rgba(0,0,0,0.5)" }}>PLAY CARD</span>
            </div>
          </div>
        )}
      </div>
