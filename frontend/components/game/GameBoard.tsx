...
export function GameBoard({
  ...
  const handleDefuse = () => {
    ...
    const dfCode = roomData.deck_config?.card_version === "good_and_evil" ? "GVE_DF" : "DF";
    playCard(dfCode); 
  };
  ...
  const closeSeeTheFuture = () => {
    // Clear state or just hide
    console.log("Closed See The Future");
  };
  ...
  return (
    <main className="flex-1 flex items-center justify-center px-6 py-4 relative">
      ...
      <SeeTheFutureModal 
        ...
        onClose={closeSeeTheFuture}
      />

      {/* ── LOG PANEL ── */}
      {roomData.status === "PLAYING" && gameLogs.length > 0 && (
        <div className="absolute left-3 top-1/2 -translate-y-1/2 w-60 z-20">
          <div className="rounded-2xl px-3 py-3 flex flex-col gap-1.5" style={{ background: "rgba(240,220,170,0.72)", backdropFilter: "blur(18px)", border: "1px solid rgba(120,70,10,0.45)", boxShadow: "0 8px 32px rgba(0,0,0,0.25)", maxHeight: "220px", overflowY: "auto" }}>
            ...
            {gameLogs.slice(-7).map((log, i, arr) => (
              <div
                ...
                style={{ color: i === arr.length - 1 ? "rgba(60,30,0,0.95)" : `rgba(80,40,0,${0.4 + i * 0.12})` }}
              >
                {log}
              </div>
            ))}
          ...
          <div className="absolute left-1/2 top-[53.5%] -translate-x-1/2 -translate-y-1/2 flex items-center justify-center gap-10">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div
                ...
                title={currentTurnSeat !== null && isMySeat(currentTurnSeat) ? "จั่วไพ่" : "ยังไม่ถึงเทิร์นของคุณ"}
